...
def search(q, maxpages=-1, expand=False, captions=False, **kwargs):
    ...
    expand   : bool(default=False)
        ...
        **currently only works for videos!**
        captions : False or Śtring
                String specifying the language for captions to retrieve (such as 'en' for english). Defaults to False (no captions)
    ...
    if captions and res:
        res['items'] = get_captions(res['items'], language=captions)
...
def get_captions(vids, part="id, snippet", language='en', *args, **kwargs):
    ...
    for vid in vids:
        ...
        if captions:
            vid['caption_content']  = _get_caption(vid,language=language)
        
    return vids

def _get_caption(vid, language, types=['standard','ASR'], *args, **kwargs):
    caption_url = 'https://www.googleapis.com/youtube/v3/captions/'
    data = {'tfmt':'srt'}
    if vid.get('captions'):
        for captype in types:
            for cap in vid['captions']:
                islang = cap.get('snippet',{}).get('language','') == language
                istype = cap.get('snippet',{}).get('trackKind','')==captype
                if islang and istype:
                    cap = get(caption_url+cap['id'], params=data, oauth=True)
                    if cap!= 'The permissions associated with the request are not sufficient to download the caption track. The request might not be properly authorized, or the video order might not have enabled third-party contributions for this caption.' and cap: return cap
    return ""
